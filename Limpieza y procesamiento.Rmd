...
PoblacionVacunada14Oct=merge(Vacuna14Oct,PoblacionCopia,by="id_persona")

PoblacionVac14Oct=aggregate(id_persona~id_ubigeo_domicilio,data=PoblacionVacunada14Oct,FUN=length)
```
...
names(PoblacionVac14Oct)[names(PoblacionVac14Oct) == 'id_persona'] <- 'Vac14Oct'


PoblacionD18=Reduce(function(...) merge(..., all=TRUE), list(PoblacionVac6Jun, PoblacionVac28Jul, PoblacionVac14Oct, PoblacionD18))
```
...
Distritos$Avance3=Distritos$Vac14Oct/Distritos$Poblacion18*100
```
...
IDH3$Mes="14 de octubre"
```

```{r warning=FALSE}
IDHVacuna=Reduce(function(...) merge(..., all=TRUE), list(IDH1,IDH2, IDH3))

IDHVacuna$Mes=ordered(IDHVacuna$Mes,levels=c("06 de junio","28 de julio","14 de octubre"))
```
...
Pobreza3=Distritos %>%
  group_by(PobrezaCategorias) %>% 
   summarise(mean =  mean(Avance3), 
             wm = sum(Poblacion18*Avance3)/sum(Poblacion18)) 
Pobreza3$Mes="14 de octubre"
```

```{r warning=FALSE}
PobrezaVacuna=Reduce(function(...) merge(..., all=TRUE), list(Pobreza1,Pobreza2,Pobreza3))

PobrezaVacuna$Mes=ordered(PobrezaVacuna$Mes,levels=c("06 de junio","28 de julio","14 de octubre"))
```
...
IVIA3= Distritos %>%
  group_by(IVIACategorias) %>% 
   summarise(mean =  mean(Avance3), 
             wm = sum(Poblacion18*Avance3)/sum(Poblacion18)) 
IVIA3$Mes="14 de octubre"
```

```{r warning=FALSE}
IVIAVacuna=Reduce(function(...) merge(..., all=TRUE), list(IVIA1,IVIA2, IVIA3))

IVIAVacuna$Mes=ordered(IVIAVacuna$Mes,levels=c("06 de junio","28 de julio","14 de octubre"))
```
